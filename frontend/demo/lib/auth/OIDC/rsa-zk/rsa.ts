...
function multiply(
  ...
  const { q, r } = Provable.witness(
    ...
    () => {
      const xy = x.toBigint() * y.toBigint();
      const p0 = p.toBigint();
      const q = xy / p0;
      const r = xy - q * p0;
  ...
  const delta: Field[] = Array.from({ length: 2 * 18 - 1 }, () => Field(0));
  const [X, Y, Q, R, P] = [x.fields, y.fields, q.fields, r.fields, p.fields];
  ...
  for (let i = 0; i < 2 * 18 - 2; i++) {
    const deltaPlusCarry = delta[i].add(carry).seal();
...
function rangeCheck116(x: Field) {
  const [x0, x1] = Provable.witnessFields(2, () => [
    x.toBigInt() & ((1n << 64n) - 1n),
    x.toBigInt() >> 64n,
  ]);

  Gadgets.rangeCheck64(x0);
  const [x52] = Gadgets.rangeCheck64(x1);
